...
// 获取cookie存储key
func (j *Jar) GetJarKey(u *url.URL) string {
	if u.Scheme != "http" && u.Scheme != "https" {
		return ""
	}
	host, err := canonicalHost(u.Host)
...
// 使用md5作为唯一标识
func (j *Jar) GetHash(u *url.URL) string {
	cookies := j.Cookies(u)
	cookiesBuffer := bytes.NewBufferString("")
	for _, v := range cookies {
		cookiesBuffer.WriteString(v.String())
	}
	h := md5.New()
	h.Write(cookiesBuffer.Bytes())
	return hex.EncodeToString(h.Sum(nil))
}

// 获取用于本地缓存的原始数据
func (j *Jar) GetEntryMap(u *url.URL) map[string]Entry {
	if u.Scheme != "http" && u.Scheme != "https" {
		return nil
	}
	host, err := canonicalHost(u.Host)
